...
import { Mic, Calendar, Plus, Users, ClipboardList, X, Check, MapPin, ChevronDown, ChevronUp, UserPlus, ArrowUpDown, BarChart3, Download } from "lucide-react";
...
export default function OralPresentationPage() {
  ...
  async function reload() {
    const data = await fetch("/api/presentations?type=ORAL").then((r) => r.json());
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/presentations?type=ORAL").then((r) => r.json()),
    ...
    if (tab === "scoring" && !scoringLoaded) {
      ...
      fetch("/api/presentations/scoring-dashboard")
        ...
        .then((res) => {
          ...
          setScoringData(all.filter((p) => p.type === "ORAL"));
        })
        .catch(() => {});
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <SummaryStatCard label={t("presentations.total")} value={filteredPresentations.length} icon={<Mic className="h-5 w-5" />} color="blue" />
